...
import pytest
from dht_query.types import InetAddr, NodeId, Node, InfoHash
...
class TestInetAddr:
    ...
    def test_parse_valid(self, input_str, expected_host, expected_port, expected_type):
    ...
    def test_from_ipstr_port_valid(self, host, port, expected_type):
    ...
    def test_from_ipstr_port_invalid(self, invalid_host):
    ...
    def test_from_compact_valid(self, host, port):
    ...
    def test_from_compact_invalid_length(self, length):
        bs = b"\x00" * length
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (socket.AF_INET6, IPv6Address("ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff"), 65535),
        ],
    )
    def test_resolve_with_ip(self, family, host, port):
    ...
    @pytest.mark.parametrize(
        "family, hostname, port, ip",
        [
            (socket.AF_INET, "github.com", 80, "140.82.121.3"),
            (socket.AF_INET, "google.com", 80, "172.217.169.142"),
            (socket.AF_INET, "cloudflare.com", 80, "140.82.121.3"),
        ]
    )
    def test_resolve_with_hostname(self, monkeypatch, family, hostname, port, ip):
        def fake_getaddrinfo(*_a, **_kw):
            return [(family, 0, 0, 0, (hostname, port))]
        ...
        assert actual_address_family == family
        assert actual_host == str(hostname)
        assert actual_port == port
    ...
    def test_resolve_invalid_or_unknown_hostname(self, monkeypatch, unknown_hostname):
        def fake_getaddrinfo(*_a, **_kw):
    ...
    def test_str(self, host, port, expected_str):
        addr = InetAddr(host=host, port=port)
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param("example.com", 80, IPv4Address("1.1.1.1"), 80, True, id="str < IPv4"),
            pytest.param(IPv4Address("1.1.1.1"), 80, IPv6Address("::1"), 80, True, id="IPv4 < IPv6"),
            pytest.param(IPv4Address("1.1.1.1"), 80, IPv4Address("1.1.1.2"), 80, True, id="IPv4 host comparison"),
            pytest.param(IPv4Address("1.1.1.1"), 80, IPv4Address("1.1.1.1"), 90, True, id="IPv4 port comparison"),
            pytest.param(IPv4Address("1.1.1.1"), 80, IPv4Address("1.1.1.1"), 80, False, id="Equal objects"),
            pytest.param(IPv6Address("::"), 80, IPv4Address("0.0.0.0"), 80, False, id="IPv6 < IPv4"),
        ]
    )
    def test_lt(self, a_host: Any, a_port: Any, b_host: Any, b_port: Any, expected):
...
class TestNodeId:
    ...
    def test_invalid_hex_number(self, invalid_hex_number):
        with pytest.raises(ValueError) as e:
...
class TestInfoHash:
    ...
    def test_init_valid(self, info_hash):
        info_hash = InfoHash(info_hash)
        assert isinstance(info_hash.hash, bytes)
        assert len(info_hash.hash) == 20

    @pytest.mark.parametrize("hash_length", [0, 1, 5, 7, 19, 50])
    def test_init_invalid_hash_length(self, hash_length):
